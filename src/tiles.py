...
def hashUNH (ints, numInts, number_actions, increment=449):
    "Hashing of array of integers into below number_actions, using random table"
    res = 0
    ...
    if ccheck == ct.data[j]:  # if new data same as saved data, add to hits
        ...
        while ccheck != ct.data[j]:  # keep looking for a new spot until we find an empty spot
            ...
            if i > memSize:  # or we run out of space 
                print "Tile: Collision table out of memory"
...
def powerOf2 (number_parameters):
    lgn = math.log(number_parameters, 2)
